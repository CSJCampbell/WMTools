...
test_that("activation aim", {
    ...
    test1 <- activation(blueleader, target = list(stats = c(DEF = 12, ARM = 18)), boost_hit = TRUE, boost_damage = TRUE, 
    ...
    test2 <- activation(redbandit, target = list(stats = c(DEF = 4 + 2 + 3 * 6, ARM = 18)), boost_hit = TRUE, boost_damage = TRUE, 
        foc = 3, dice = rep(6, 6))
    expect_equal(object = test2, expected = 14)

    # TEST 3: aim 5, 6, 6 vs DEF 24
    test3 <- activation(redbandit, target = list(stats = c(DEF = 4 + 2 + 3 * 6, ARM = 18, BASE = 30)), boost_hit = TRUE, boost_damage = TRUE, 
        foc = 3, dice = c(5, 6, 6, 4, 6))
    expect_equal(object = test3, expected = 0)

    # TEST 4: aim all sixes, 3 FOC, ROF 3
    test4 <- activation(list(stats = c(RAT = 6), range = list(gun = list(stats = c(RNG = 10, ROF = 3, AOE = NA, POW = 10), special = ""))), 
        target = list(stats = c(DEF = 20, ARM = 21, BASE = 30)), boost_hit = FALSE, boost_damage = FALSE, 
